    ...
    for channel in yp.channels:
        ...
        if channel.type in {'RGB', 'VALUE'}:
            ...
            if not yp.use_linear_blending and channel.colorspace != 'LINEAR' and any_layers_using_channel(channel):
                if channel.type == 'RGB':
                    start_linear = check_new_node(group_tree, channel, 'start_linear', 'ShaderNodeGamma',
                                                  'Start Linear')
                else:
            ...
            if channel.type == 'RGB':
                ...
                if not yp.use_linear_blending and (
                        channel.colorspace != 'LINEAR' or channel.use_clamp) and any_layers_using_channel(channel):
                    end_linear = check_new_node(group_tree, channel, 'end_linear', 'ShaderNodeMath',
                                                'End Linear & Clamp')
            ...
            if end_max_height_tweak:
                if 'Height Tweak' in end_max_height_tweak.inputs: channel.height_tweak = end_max_height_tweak.inputs[
                    'Height Tweak'].default_value

                # Rename fcurve datapath
                for fcs in get_action_and_driver_fcurves(group_tree):
                    for fc in fcs:
                        match = re.match(
                            r'^nodes\["' + channel.end_max_height_tweak + r'"\]\.inputs\[(\d+)\]\.default_value$',
                            fc.data_path)
                        if match:
            ...
            if not is_bl_newer_than(3) and channel.enable_subdiv_setup:
                if not is_bl_newer_than(2, 80):
                    ...
                    lib_name = lib.CHECK_INPUT_NORMAL_MIXED
            ...
            if not is_bl_newer_than(5) or channel.enable_subdiv_setup or not any_bumps or any_layers_using_normal_map(
                    channel) or channel.enable_smooth_bump:
                ...
                if channel.enable_smooth_bump:
                    if is_normal_height_input_connected(channel):
                        if channel.enable_subdiv_setup:  # and ypup.eevee_next_displacement:
                            ...
                            process_lib_name = lib.FINE_BUMP_PROCESS_START_BUMP
                    else:
            ...
            if yp.use_baked and channel.enable_subdiv_setup and (
                    any_layers_using_disp(channel) or any_layers_using_vdisp(channel)):
            ...
            if end_linear and 'Normal Tweak' in end_linear.inputs and (
                    ...
                    or (
                            channel.enable_smooth_bump and channel.enable_smooth_normal_tweak and process_lib_name != '' and end_linear.node_tree.name != process_lib_name)
            ):
                    ...
                    for fcs in get_action_and_driver_fcurves(group_tree):
                        for fc in fcs:
                            if fc.data_path == channel.path_from_id() + '.smooth_normal_tweak':
                                ...
                                fc.data_path = 'nodes["' + end_linear.name + '"].inputs[' + str(
                                    index) + '].default_value'
...
def check_all_channel_ios(yp, reconnect=True, specific_layer=None, remove_props=False, force_height_io=False,
    ...
    for ch in yp.channels:
        ...
        if ch.type == 'NORMAL' and (ch.enable_subdiv_setup or force_height_io):
            ...
            create_input(group_tree, name, 'NodeSocketVector', valid_inputs, input_index, default_value=(0, 0, 0),
                         hide_value=True)
            input_index += 1
    ...
    for layer in yp.layers:
        ...
        check_all_layer_channel_io_and_nodes(layer, specific_ch=specific_ch, do_recursive=False,
                                             remove_props=remove_props, hard_reset=hard_reset)
...
def check_all_layer_channel_io_and_nodes(layer, tree=None, specific_ch=None, do_recursive=True, remove_props=False,
                                         hard_reset=False):  # , check_uvs=False): #, has_parent=False):
...
def check_layer_tree_ios(layer, tree=None, remove_props=False, hard_reset=False):
    ...
    if not remove_props and layer_enabled:
        ...
        if is_bl_newer_than(2, 81) and layer.enable_uniform_scale and is_layer_using_vector(
                layer) and layer.segment_name == '':
        ...
        for i, ch in enumerate(layer.channels):
            ...
            if alpha_ch != ch or (alpha_ch == ch and (
                    not get_channel_enabled(color_ch) or color_ch.unpair_alpha or layer.type == 'GROUP')):
            ...
            if root_ch.type == 'NORMAL':
                ...
                if ch.enable_transition_bump:
                    ...
                    if ch.transition_bump_falloff and ch.transition_bump_falloff_type == 'EMULATED_CURVE':
                        dirty = create_prop_input(ch, 'transition_bump_falloff_emulated_curve_fac', valid_inputs,
                                                  input_index, dirty, float_factor_input_names)
                        input_index += 1
        ...
        for mask in layer.masks:
            ...
            if is_bl_newer_than(2, 81) and mask.enable_uniform_scale and is_mask_using_vector(
                    mask) and mask.segment_name == '':
            ...
            if root_ch.type == 'NORMAL' and layer.type == 'GROUP':
                ...
                if is_height_process_needed(layer):
                    ...
                    if root_ch.enable_smooth_bump:
                        ...
                        for letter in nsew_letters:
                            name = root_ch.name + io_suffix['HEIGHT_' + letter.upper()] + io_suffix['ALPHA'] + \
                                   io_suffix['GROUP']
                            dirty = create_input(tree, name, 'NodeSocketFloat', valid_inputs, input_index, dirty)
                            input_index += 1
    ...
    for mask in layer.masks:
        if get_mask_enabled(mask, layer) and mask.texcoord_type not in {'UV', 'Decal', 'Layer'} and mask.type not in {
            'VCOL', 'COLOR_ID', 'OBJECT_INDEX', 'HEMI'} and mask.texcoord_type not in texcoords:
            ...
            if inp.name.startswith('.'):
                ...
                if m:
                    ...
                    if socket_type in {'NodeSocketColor', 'RGBA'}:
                        ...
                        try:
                            setattr(root_tree.path_resolve(entity_path), prop_name, (val[0], val[1], val[2]))
                        ...
                        try:
                            setattr(root_tree.path_resolve(entity_path), prop_name, val)
                        except Exception as e:
                            print(e)
