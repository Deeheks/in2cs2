...
if not is_bl_newer_than(3, 2):
    ...
    DEFAULT_NEW_VCOL_SUFFIX = ' Attribute'
...
def add_new_layer(
    ...
    if layer_type == 'IMAGE':
        ...
        if vcol:
            ...
            set_source_vcol_name(source, layer_name)
            ...
            if mask_type == 'VCOL':
                ...
                if mask_vcol_fill and bpy.context.mode == 'EDIT_MESH':
                    bpy.ops.mesh.y_vcol_fill_face_custom(
                        color=(mask_color_id[0], mask_color_id[1], mask_color_id[2], 1.0))
...
class YUseLinearColorSpace(bpy.types.Operator):
    ...
    @classmethod
    def poll(cls, context):
        return hasattr(context,
                       'layer')  # and hasattr(context, 'channel') and hasattr(context, 'image') and context.image
...
class YNewVcolToOverrideChannel(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if ch.override_type == 'VCOL':
            ...
            vcol_node, dirty = check_new_node(tree, ch, 'cache_vcol', get_vcol_bl_idname(), '', True)
...
class YNewLayer(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        while True:
            ...
            self.mask_color_id = (random.uniform(COLORID_TOLERANCE, 1.0), random.uniform(COLORID_TOLERANCE, 1.0),
    ...
    def check(self, context):
        ...
        if self.use_image_atlas:
            if self.hdr:
                max_size = ypup.hdr_image_atlas_size
            else:
                max_size = ypup.image_atlas_size
        ...
        if self.use_image_atlas_for_mask:
            if self.mask_use_hdr:
                mask_max_size = ypup.hdr_image_atlas_size
            else:
                mask_max_size = ypup.image_atlas_size
    ...
    def get_to_be_cleared_image_atlas(self, context, yp):
        ...
        if self.add_mask and self.mask_type == 'IMAGE' and self.use_image_atlas_for_mask:
            return ImageAtlas.check_need_of_erasing_segments(yp, self.mask_color, self.mask_width, self.mask_height,
                                                             self.hdr)
    ...
    def execute(self, context):
        ...
        if self.type == 'IMAGE':
            ...
            same_name = [lay for lay in yp.layers if lay.name == self.name]
        if same_name:
            if self.type == 'IMAGE':
        ...
        if not is_bl_newer_than(2, 93) and (
                self.type in {'EDGE_DETECT', 'AO'} or self.add_mask and self.mask_type == 'EDGE_DETECT'):
        ...
        if self.type == 'IMAGE':
            ...
            if self.use_image_atlas:
                if self.use_udim:
                    segment = UDIM.get_set_udim_atlas_segment(tilenums, self.width, self.height, color, colorspace,
                                                              self.hdr, yp)
                else:
                    segment = ImageAtlas.get_set_image_atlas_segment(self.width, self.height, 'TRANSPARENT', self.hdr,
                                                                     yp=yp, colorspace=colorspace)
                img = segment.id_data
            else:
        ...
        layer = add_new_layer(
            ...
            use_image_atlas_for_mask=self.use_image_atlas_for_mask,
            ...
            mask_vcol_data_type=self.mask_vcol_data_type, mask_vcol_domain=self.mask_vcol_domain,
            use_divider_alpha=self.use_divider_alpha,
            use_udim_for_mask=self.use_udim_for_mask, interpolation=self.interpolation,
...
class YOpenImageToOverrideChannel(bpy.types.Operator, ImportHelper, BaseOperator.OpenImage):
    ...
    def execute(self, context):
        ...
        if root_ch.type == 'NORMAL':
            for img in images:
                ...
                if (('normal' in img_name or 'norm' in img_name or img_name.endswith(('_nor', '.nor', '_n', '.n')))
                        and 'displacement' not in img_name
                # Baked displacement from ucupaint can contains 'normal' word
                ):
                    image_1 = img
                elif not image:
                    image = img
        ...
        if image_1:
            ...
            if self.relative:
                try:
                    image_1.filepath = bpy.path.relpath(image_1.filepath)
                except:
                    pass
    ...
    def execute(self, context):
        ...
        if image_1:
            ...
            if ch.override_type == 'IMAGE':
                source_tree = get_channel_source_tree(ch, layer)
                source_label = root_ch.name + ' Override : ' + ch.override_type
                image_node_1, dirty = check_new_node(source_tree, ch, 'source', 'ShaderNodeTexImage', source_label,
                                                     True)
            else:
...
class BaseMultipleImagesLayer(BaseOperator.OpenImage):
    ...
    def open_images_to_single_layer(self, context: bpy.context, directory: str = '', import_list=[],
                                    non_import_images=[], channel_image_dict={}) -> bool:
                ...
                for image in images:
                    ...
                    if self.is_synonym_in_image_name(syname, alias):
                        ...
                        if (ch.type != 'NORMAL' or
                                ...
                                (syname not in bump_synonyms and not normal_image_found)
                        # Only proceed if normal image is not yet found
                        ):
        ...
        elif any([ch for i, ch in enumerate(valid_channels) if
                  ch.type == 'NORMAL' and valid_synonyms[i] in {'normal', 'n'}]):
        ...
        for i, image in enumerate(valid_images):
            ...
            if i == 0:
                ...
                layer = add_new_layer(
                    ...
                    add_mask=self.add_mask, mask_type=self.mask_type, mask_color=self.mask_color,
                    mask_use_hdr=self.mask_use_hdr,
                ...
                if root_ch.type == 'NORMAL' and (
                        syname in {'normal', 'n'} or 'normal without bump' in image.name.lower()):
                    if image == main_image:
                        ch.override_1 = False
                    else:
                        image_node, dirty = check_new_node(tree, ch, 'cache_1_image', 'ShaderNodeTexImage', '', True)
                        ...
                        ch.override_1 = True
                        ch.override_1_type = 'IMAGE'
                    if (self.normal_map_flip_y and (not gl_image or gl_image != image)) or (
                            dx_image and dx_image == image):
...
class YOpenImagesFromMaterialToLayer(bpy.types.Operator, ImportHelper, BaseMultipleImagesLayer):
    ...
    def draw(self, context):
        ...
        if self.asset_library_path == '':
            ...
            row.label(text=self.mat_name, icon='MATERIAL_DATA')
    ...
    def execute(self, context):
        ...
        if not self.open_images_to_single_layer(context, directory='', import_list=[], non_import_images=images,
                                                channel_image_dict=channel_image_dict):
        ...
        if self.type == 'IMAGE':
            ...
            if should_be_normal:
                ...
                if ch.override_type == 'IMAGE':
                    source_tree = get_channel_source_tree(ch, layer)
                    source_label = root_ch.name + ' Override : ' + ch.override_type
                    image_node, dirty = check_new_node(source_tree, ch, 'source', 'ShaderNodeTexImage', source_label,
                                                       True)
                else:
                    image_node, dirty = check_new_node(tree, ch, 'cache_image', 'ShaderNodeTexImage', '', True)
...
def draw_remove_group(self, context):
    ...
    if hasattr(context, 'layer') and any_single_user_ondisk_image_inside_group(context.layer):
        ...
        c = col.operator("wm.y_remove_layer",
...
def replace_layer_type(layer, new_type, item_name='', remove_data=False):
    ...
    cache = None
    if new_type not in {'IMAGE', 'VCOL', 'BACKGROUND', 'GROUP', 'HEMI', 'EDGE_DETECT', 'AO'} or (
            new_type in {'IMAGE', 'VCOL'} and item_name == ''):
    ...
    if layer.type == 'IMAGE':
        ...
        layer.name = get_unique_name(layer.name.replace(layer_type_labels[ori_type], layer_type_labels[layer.type]),
                                     yp.layers)
    ...
    layer.expand_source = layer.type not in {'IMAGE', 'VCOL'} or (
                image != None and image.y_bake_info.is_baked and not image.y_bake_info.is_baked_channel)
...
def duplicate_layer_nodes_and_images(tree, specific_layers=[], packed_duplicate=True, duplicate_blank=False,
                                     ondisk_duplicate=False, set_new_decal_position=False):
    yp = tree.yp
    ypup = get_user_preferences()
...
class YPasteLayer(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for lname in relevant_layer_names:
            ...
            if not matching:
                ...
                for root_ch in yp.channels:
                    ...
                    new_ch.enable = new_layer.type in {'GROUP',
                                                       'BACKGROUND'}  # Layer channel default is disabled except for group and background
        ...
        if self.any_baked and self.rebake_bakeds and is_bl_newer_than(2, 80):
            ...
            self.report({'INFO'},
                        'Rebaking pasted layers is done in ' + '{:0.2f}'.format(time.time() - T) + ' seconds!')
...
def update_uv_name(self, context):
    ...
    if smooth_bump_ch and smooth_bump_ch.enable and (
            smooth_bump_ch.normal_map_type in {'BUMP_MAP', 'BUMP_NORMAL_MAP'} or smooth_bump_ch.enable_transition_bump):
...
class YLayer(bpy.types.PropertyGroup, Decal.BaseDecal):
    ...
    divide_rgb_by_alpha: BoolProperty(
        name='Divide RGB by Alpha',
        description="Dividing RGB value by its alpha\nThis can be useful remove dark outline on painted image/" + get_vertex_color_label(
