...
def draw_root_channels_ui(context, layout, node):
    ...
    channel = yp.channels[yp.active_channel_index] if len(yp.channels) > 0 and yp.active_channel_index < len(
        yp.channels) else None
...
def draw_layers_ui(context, layout, node):
    ...
    # [HACK] Cache is necessary to improve performance since blender always update the UI in modal operation and when using the sliders
    ...
    if channel_mismatch:
        row = box.row(align=True)
        row.alert = True
        row.operator("wm.y_fix_channel_missmatch", text='Fix Mismatched Channels!', icon='ERROR')
        row.alert = False
        return
...
def main_draw(self, context):
    ...
    if ypui.show_bake_targets:
        if len(yp.bake_targets) == 0:
            ...
            if is_bl_newer_than(2, 80):
                rrow.alignment = 'RIGHT'
            rrow.operator('wm.y_new_bake_target_setup', text='Setup for CS2').preset = "CS2"
...
class YPAssetBrowserMenu(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        active_mat = get_active_material(obj)
...
def draw_ypaint_about(self, context):
    ...
    if is_bl_newer_than(2, 80) and is_online() and member_count > 0:
        ...
        row_title_label.label(text='ucupaint is created by:')
    ...
    col.operator('wm.url_open', text='ucupaint Wiki', icon='TEXT').url = 'https://ucupumar.github.io/ucupaint-wiki/'
    col.operator('wm.url_open', text='ucupaint GitHub', icon='SCRIPT').url = 'https://github.com/ucupumar/ucupaint'
    icon = 'COMMUNITY' if is_bl_newer_than(2, 80) else 'SEQ_SEQUENCER'
    col.operator('wm.url_open', text='ucupaint Discord Server', icon=icon).url = 'https://discord.gg/BdNfGGzQHh'
...
class YBakeListSpecialMenu(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        col.operator('wm.y_paste_bake_target', text="Paste Texture Values", icon='PASTEDOWN').paste_as_new = False

class YBakeTargetMenu(bpy.types.Menu):
    bl_idname = "NODE_MT_y_bake_target_menu"
    bl_description = 'Texture Menu'
...
class YBakeTargetUI(bpy.types.PropertyGroup):
    expand_content : BoolProperty(
        ...
        description = 'Expand Texture options',
        default = True,
    ...
    expand_r : BoolProperty(
        name = 'R Channel',
        description = 'Expand Texture R channel options',
        default = False,
        update = update_bake_target_ui
    )
    ...
    expand_b : BoolProperty(
        name = 'B Channel',
        description = 'Expand Texture B channel options',
        default = False,
        update = update_bake_target_ui
    )
    ...
    expand_a : BoolProperty(
        name = 'A Channel',
        description = 'Expand Texture A channel options',
        default = False,
        update = update_bake_target_ui
    )
...
class YPaintUI(bpy.types.PropertyGroup):
    ...
    show_bake_targets : BoolProperty(
        name = 'Textures',
        description = 'Show Texture lists',
        default = False
    )
    ...
    # Textures (Bake targets) related UI
    ...
    for attr in dir(source):
        if attr.startswith(('show_', 'expand_')) or attr.endswith('_name'):
            try: setattr(dest, attr, getattr(source, attr))
                ...
                print('EXCEPTION: Cannot set UI settings!')
