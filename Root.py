...
class YQuickYPaintNodeSetup(bpy.types.Operator, BaseOperator.BlendMethodOptions):
    ...
    def invoke(self, context, event):
        ...
        # Set the tree name
        #self.tree_name = YP_GROUP_PREFIX + obj.name
        self.tree_name = obj.name
    ...
    def draw(self, context):
        ...
        col.prop(self, 'create_cs2_baketargets')
    ...
    def execute(self, context):
        ...
        if len([ng for ng in bpy.data.node_groups if hasattr(ng, 'yp') and ng.yp.is_ypaint_node]) == 1:
            context.window_manager.ypui.show_materials = False
            context.window_manager.ypui.expand_channels = True

        # Create CS2 Textures
        if self.create_cs2_baketargets:
            bpy.ops.wm.y_new_bake_target_setup(preset='CS2')
            context.window_manager.ypui.show_bake_targets = True
...
class YRenameYPaintTree(bpy.types.Operator):
    ...
    rename_active_images: BoolProperty(name='Also Rename Textures', description='Also rename Textures', default=True)
    ...
    def draw(self, context):
        ...
        row.prop(self, 'name')
        rrow = self.layout.row(align=True)
        rrow.prop(self, 'rename_active_images', text='Also rename Textures')
